...
if(isMarketingEmployee(current)){
  ...
  employeePageDescription.textContent="كل عميل يرسله المدير يظهر هنا كمهمة تواصل مستقلة.";
...
function renderMarketing(tasks){
  ...
  const empty=!tasks.length;employeeEmpty.classList.toggle("hidden",!empty);employeeTableWrap.classList.toggle("hidden",empty);
...
function renderProjects(tasks){
  ...
  const empty=!tasks.length;projectEmpty.classList.toggle("hidden",!empty);projectTableWrap.classList.toggle("hidden",empty);
