db.ref("tasks").on("value", snap => {
  ...
  document.getElementById("dashboardBody").innerHTML = employees.map(e=>{
    ...
    return `<tr><td><div class="employee-name"><div class="avatar">${esc(e.name[0])}</div><strong>${esc(e.name)}</strong></div></td><td>${s.total}</td><td>${s.contacted}</td><td>${s.won}</td><td><strong>${s.pct}%</strong><div class="progress"><span style="width:${s.pct}%"></span></div></td></tr>`;
  }).join("");
